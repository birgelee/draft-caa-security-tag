...
Serving "security" CAA records over authenticated DNS channels is critical to the security of the records because a High-Security-Validation CAA record not protected by authenticated DNS may be suppressed by an adversary that can manipulate DNS responses. This could potentially allow the adversary to downgrade validation to use a non-high-security method and undermine the security properties of High-Security-Validation. 
